...
def generate_group_stats_card(
    ...
    draw.text((left_x + 2, 112), fit_text(draw, f"Window: {ascii_safe(time_text)}", sub_font, 620), font=sub_font, fill=(161, 203, 235))

    draw.text((left_x, 175), fit_text(draw, f"Callers {callers_count} • Calls {total_calls}", block_font, 620), font=block_font, fill=(255, 255, 255))
    draw.text((left_x, 228), fit_text(draw, f"Hit Rate {win_rate_pct:.1f}%", stat_font, 610), font=stat_font, fill=(141, 255, 113))
    draw.text((left_x, 276), fit_text(draw, f"Average {avg_text}", stat_font, 610), font=stat_font, fill=(141, 255, 113))
...
def generate_daily_digest_card(digest_data):
    ...
    overlay = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    od = ImageDraw.Draw(overlay)
    od.rounded_rectangle((28, 24, 1172, 416), radius=28, fill=(13, 20, 43, 185), outline=(98, 121, 186, 145), width=2)
    od.ellipse((760, -90, 1280, 350), fill=(96, 165, 250, 45))
    card = Image.alpha_composite(card.convert("RGBA"), overlay).convert("RGB")
    draw = ImageDraw.Draw(card)
    ...
    draw.text((left_x, 175), fit_text(draw, f"Calls {digest_data['total_calls']} • Callers {digest_data['total_callers']}", block_font, 620), font=block_font, fill=(255, 255, 255))

    top = digest_data["top"][0] if digest_data["top"] else None
    if top:
        draw.text((left_x, 228), fit_text(draw, f"Top: {ascii_safe(top['name'])}", stat_font, 620), font=stat_font, fill=(141, 255, 113))
        draw.text((left_x, 276), fit_text(draw, f"Avg {format_return(top['avg_now_x'])} • Win {top['win_rate']:.1f}%", block_font, 620), font=block_font, fill=(217, 236, 255))
    else:
